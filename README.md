...
**`Added Consonant part`** : Program extended to return the number of consonants for each string array passed. Added consonantCount constructor variable  in CountResult class. which return vowel's count, consonant count in the same method.
**`BDD Cucumber feature :** `Project is updated as per cucumber format. Feature file is added under src/test/java
**`Hooks,Stepdefinition,Runner :`** All files is added under src/test/java/com/exercise.

`How to run the test :` Run the runnerTest file under src/test/java/com/exercise. Individually you can run cucumber tests as well.

**`Improvement/Bug :**` For vowel counter we added condition equal to 4 but as per acceptance criteria 4 string accepted. So the code changed accordingly.
